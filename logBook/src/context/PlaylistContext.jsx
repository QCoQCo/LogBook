...
export const PlaylistProvider = ({ children }) => {
    ...
    const findUserIdByPlayId = useCallback(
        (playId) => {
            ...
            for (const [uid, lists] of entries) {
                if (!isNaN(uid) && lists.some((pl) => String(pl.playId) === String(playId)))
                    return uid;
            }
            // 2순위: 그 외(아이디 등)에서 찾음
            for (const [uid, lists] of entries) {
    ...
    const fetchPlaylists = useCallback(async (userId) => {
        ...
        try {
            ...
            const mappedLists = serverLists.map((pl) => ({
                ...
                userId: Number(pl.userId),
                ownerLoginId: pl.ownerLoginId,
                title: pl.title,
                songs: (pl.items || [])
                    .sort((a, b) => (a.seq || 0) - (b.seq || 0)) // 클라이언트 측 2차 정렬 보장
                                        .map((item) => ({
                                            id: item.id.toString(), // [추가] 팝업창 표준 식별자
    ...
    const fetchPlaylistDetail = useCallback(async (playId) => {
        ...
        try {
            ...
            const mapped = {
                playId: pl.id.toString(),
                userId: Number(pl.userId), // [강화] PK는 숫자로 유지
                ownerLoginId: pl.ownerLoginId,
                title: pl.title,
    ...
    const updatePlaylistSongs = useCallback(
        async (userId, playId, newSongs) => {
            ...
            try {
                // [수정] 개별 루프 호출 대신 일괄 업데이트 API 사용 (N+1 문제 해결)
                const batchItems = newSongs.map((song, i) => ({
                    id: Number(song.contentId), // 백엔드 아이템 PK (Long)
                    seq: i,
                    title: song.title,
                    link: song.link,
                    thumbnail: song.thumbnail,
                ...
                await playlistService.updatePlaylistItemsBatch(playId, batchItems);
